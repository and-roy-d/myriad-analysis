...
(ctrlurl, dataurl, modampl, bias) = ('udp://10.0.15.16#fset3', 'udp://10.0.15.16', float(sys.argv[1]), sys.argv[2])
...
#modflist = np.logspace(1, 5.3, num=50)
modflist = np.array([fr/2**16, fr/2**12, fr/2**8, fr/2**4, fr/2])
modflist = fr/2**np.arange(1, 16)[::-1]
...
plot_idx = [0, len(modflist)-2, len(modflist)-1]
plotchan = 0 #channel to plot for debugging
abaco_atten = 10 #attenuation on theoutput of abaco (*not* dB ... actual reduction factor)
...
for f in range(len(modflist)):
    ...
    print("tonegen.set_q()", modfreq, modampl)
    #tonegen.dumpfields()
    if modfreq>1e3:
    ...
    for ch in range(len(atan1[0,:])):
        ...
        if ch == plotchan and f in plot_idx:
            ...
            pl.plot(fit_t, fit_ch, label="fit_cos")
...
pl.ylabel("Ampl")
pl.subplot(2, 1, 2)
pl.semilogx(modflist, np.unwrap(shift_tot[plotchan,:]-np.pi)*180/np.pi, "o-", label="shift_tot")
...
pl.semilogx(modflist, phi_fit_dtb[plotchan,:]*180/np.pi, "o-", label="dtb")
pl.legend()
pl.xlabel("Frequency (Hz)")
