...
import sys, time
import qsdumux.core.helpers as corehelpers
import qsdumux.core.interfaces as interfaces
import qsdumux.core.packetparser as packetparser
import qsdumux.fpgaip.helpers as fpgaiphelpers
import datetime
import os
import numpy as np
import pylab as pl
...
#(ctrlurl, dataurl, freq, modampl, toread) = ('pci://0000:17:00.0/fset0', 'udp://10.0.0.10', 'float(100)', '.005', '8192')

#(ctrlurl, dataurl, freq, modampl, toread) = (sys.argv[1], sys.argv[2], float(sys.argv[3]), float(sys.argv[4]), int(sys.argv[5]))

(ctrlurl, dataurl, modampl, bias) = ('udp://10.0.15.36#fset0', 'udp://10.0.15.36', float(sys.argv[1]), sys.argv[2])
...
def fit_cos_dtb(ang, t, sync_idx, f0):
    print('fit_cos_dtb', f0, len(sync_idx[0]))
    offset = np.mean(ang)
    if len(sync_idx[0]) < 10:
        raise Exception("Not enough periods %s" % sync_idx)
    idx = np.arange(sync_idx[0][0], sync_idx[0][10])
    nper = len(sync_idx[0]) - 1
    mix_cos = np.cos(2*pi*modflist[f]*(t[idx] - t[idx[0]]))
    mix_sin = np.sin(2*pi*modflist[f]*(t[idx] - t[idx[0]]))
    ...
    out = ([amp_dtb, phase_dtb + pi/2, offset, f0], [])
    return out, t, fit_t, fit, idx
        
fr = 1e6/256/8*1e3

#frequencies to measure
#modflist = np.logspace(1, 5, num=35)
modflist = np.logspace(1, 5.3, num=50)
modflist = np.array([10, 100, 1e3, 10e3, 100e3, 200e3])
modflist = np.array([122070.3125, 130e3])
modflist = np.array([90e3, 97.656e3, 100e3, 110e3, 120e3, 122e3, 130e3])
#modflist = np.array([ 100, 200e3, 240e3])
#modflist = np.logspace(1, 5.3, num=50)
modflist = np.array([ fr/20 ])
modflist = np.array([ fr / 3 ])
modflist = np.array([ fr / 8 ])
#modflist = np.array([ 25001*fr/2**16 ])


# We aim to fit 10 periods, and want 30 periods to fit into an exact number of flux ramps
#modflist = 10*fr/np.round(10*fr/modflist)

plot_idx = [0, 1, 2, 3, 4, 5]
plot_idx = [0]
plotchan = 0 #channel to plot for debugging
abaco_atten = 1 #attenuation on theoutput of abaco (*not* dB ... actual reduction factor)
sine_amp_out_mV = 1000*modampl/abaco_atten #the actual sine wave voltage in mV
t_min = 40*1/modflist
#t_min = np.array([8.0])  #4000*1/modflist
t_samp = 1/fr
npack_min = np.ceil(t_min/t_samp/55)  #minimum number of packets to grab per frequency point
#npack_min_hf = np.ceil(t_min/t_samp)*10
npack_min_hf = npack_min
date_str = datetime.datetime.fromtimestamp(time.time()).strftime("%Y%m%d")
time_str = datetime.datetime.fromtimestamp(time.time()).strftime("%H%M%S")
...
backend.write_pktsrc("ATAN0", "monitor")
#backend.write_monmerge(32)
backend.write_monmerge(55)
...
for f in range(len(modflist)):
    ...
    #set frequency
    # DTB - apparently duty=0.5 makes it a cosine wave ... i.e, the
    # phase = 0 point corresponds to the falling edge of the sync
    # signal
    modfreq = float(modflist[f])
    tonegen.set_q(modfreq, modampl, phase=0., mode="ddstaylor-q", duty=0.5)
    print("set_q params", modfreq, modampl)
    #tonegen.dumpfields()
    if modfreq>1e3:
    ...
    toread = 100
    print("***** toread", toread)
    (nbytes, buffer) = dataif.readall(toread*8192)
    if f == 0: print(nbytes, buffer[0:31])

    (headeroffsets, payloadoffsets, payloadlengths, seqnos) = packetparser.findlongestcontinuous(buffer, incr=8192)
    ...
    pl.figure(1)
    pl.clf()
    pl.plot(alldata.data["pos"][:, 0])
    pl.plot((alldata.data["syncbus"][:, 0] & 32)/32*31)
    ...
    if f==0:
        ...
        shift_tot_dtb = np.zeros((len(atan1[0,:]), len(modflist)))
    ...
    for ch in range(len(atan1[0,:])):
        ...
        if modfreq<100:
            ...
            mfreq=modfreq
        #(out_tmp, t, fit_t, fit_ch, _) = fit_cos_dtb(atan1.T[ch], times, sync_ind, modfreq)
        #mfreq=modfreq
        ...
        if ch==plotchan and f in plot_idx:
            """
            pl.figure();
            pl.plot(times, atan1[:,ch], 'o')
            pl.plot(times, sync[:, 0], label="sync ch0")
            pl.plot(times, sync[:, 5], label="sync ch5")
            pl.plot(times[sync_ind[0]], atan1[sync_ind[0], ch], 'ks')
            pl.xlabel("Time (s)")
            pl.title('ch %d, measured frq %f'%(ch, mfreq))
            #pl.plot(fit_t_2, fit_ch_2, label="fit_cos_dtb")
            pl.legend()
            """
            pass
        ...
        #print("phase_sync1", phase_sync1)
        #print("sync2", sync2)
        #print("phase_sync_avg", phase_sync_avg)
        ...
        if out_tmp_2[0][0]<0:
            print('flip')
            ampl_dtb = out_tmp_2[0][0]*-1
            phi_cos_dtb = out_tmp_2[0][1] - np.pi
        else:
        ...
        shift_total_dtb = phase_sync_avg + phi_cos_dtb
        if shift_total_dtb > 2*np.pi:
            shift_total_dtb = shift_total_dtb - 2*np.pi
        ...
        fmeas[ch, f] = mfreq
        ampls_dtb[ch,f] = ampl_dtb
        phi_fit_dtb[ch,f] = phi_cos_dtb
        shift_tot_dtb[ch,f] = shift_total_dtb
...
for ch in range(len(atan1[0,:])):
    pl.figure(99)
    Y[ch,:] = ampls[ch,:]*np.exp(1j*(shift_tot[ch,:]-np.pi))
    ...
    pl.plot(np.imag(Y_dtb[ch,:]), np.real(Y_dtb[ch,:]), 'o-')
    pl.xlabel("Imag(Y)")
    pl.ylabel("Real(Y)")
    pl.figure(102)
    pl.semilogx(modflist,np.imag(Y[ch,:]), 'o-')
    pl.semilogx(modflist,np.imag(Y_dtb[ch,:]), 'o-')
    pl.xlabel("Frequency (Hz)")
    pl.ylabel("Imag(Y)")
    pl.figure(103)
    pl.semilogx(modflist,np.real(Y[ch,:]), 'o-')
    pl.semilogx(modflist,np.real(Y_dtb[ch,:]), 'o-')
    pl.xlabel("Frequency (Hz)")
    pl.ylabel("Real(Y)")
...
for ch in [0]:
    ...
    Y_dtb[ch,:] = ampls_dtb[ch,:]*np.exp(1j*(phi_fit_dtb[ch, :]))
    #Y[ch,:] = Y[ch,:]*np.exp(1j*np.angle(func_1pole(modflist, 1, 2.95e3)))
    pl.plot(np.real(Y[ch,:]), np.imag(Y[ch,:]), 'o-')
    pl.plot(np.real(Y_dtb[ch,:]), np.imag(Y_dtb[ch,:]), 'o-')
    pl.xlabel("Real(Y)")
    pl.ylabel("Imag(Y)")
    pl.figure(102)
    pl.semilogx(modflist, np.abs(Y[ch,:]), 'o-')
    pl.semilogx(modflist, np.abs(Y_dtb[ch,:]), 'o-')
    pl.xlabel("Frequency (Hz)")
    pl.ylabel("Abs(Y)")
    pl.figure(103)
    pl.semilogx(modflist, 180/pi*np.angle(Y[ch,:]), 'o-')
    pl.semilogx(modflist, 180/pi*np.angle(Y_dtb[ch,:]), 'o-')
    pl.xlabel("Frequency (Hz)")
    pl.ylabel("Phase(Y) (deg)")


#plot amplitude and phase vs frequency
pl.figure()
pl.subplot(2, 1, 1)
pl.loglog(modflist, ampls[plotchan,:], 'o-')
pl.loglog(modflist, ampls_dtb[plotchan,:], 'o-')
pl.xlabel("Frequency (Hz)")
pl.ylabel("Ampl")
pl.subplot(2, 1, 2)
pl.semilogx(modflist, np.unwrap(shift_tot[plotchan,:]-np.pi)*180/np.pi, "o-")
#pl.semilogx(modflist, (shift_tot_dtb[plotchan,:]-np.pi)*180/np.pi, "o-")
pl.semilogx(modflist, np.unwrap(phi_fit_dtb[plotchan,:])*180/np.pi + 90, "o-")
pl.xlabel("Frequency (Hz)")
pl.ylabel("Shift_tot (deg)")



#np.savez('/data/20220223/cz/%s_czb2_%sV_%dmVpp_70mK.npz'%(date_time_str, bias, sine_amp_out_mV), shift_tot=shift_tot, phi_fit=phi_fit, ampls=ampls, modflist=modflist, fr=fr, npack_min=npack_min, fmeas=fmeas)

#
# Save cz data
#
#fname = '%s/cz_%s_%dmVpp.npz' % (cfg.cz_dir, cfg.date_time_str, sine_amp_out_mV)
fname = '%s/cz_%s_high1.npz' % (cfg.cz_dir, cfg.date_time_str)
print('filename: %s' % fname)
#np.savez(fname, shift_tot=shift_tot, phi_fit=phi_fit, ampls=ampls, modflist=modflist, fr=fr, npack_min=npack_min, fmeas=fmeas, ampls_dtb=ampls_dtb, phi_fit_dtb=phi_fit_dtb)
