...
#(headeroffsets, payloadoffsets, payloadlengths, seqnos) = packetparser.find(buffer, continuous=False, incr=1024)
#for offset in sorted(set(headeroffsets[:6] + headeroffsets[-2:])):
#    (consumed, p) = packetparser.parse(buffer[offset:])
#    print("%10d %08x %10d %5d %10s x %s" % (offset, p.srcid, p.seqno, p.payloadlen, p.data.shape, p.data.dtype))
#print()
#
#print("every packet:", toread, "->", nbytes, "=", len(payloadoffsets), "packets =", sum(payloadlengths), "byte total payload")
#print()
...
#(headeroffsets, payloadoffsets, payloadlengths, seqnos) = packetparser.findlongestcontinuous(buffer, incr=512)
#if debug: print(headeroffsets, payloadoffsets, payloadlengths, seqnos)
#(consumed, alldata) = packetparser.parsemany(buffer, headeroffsets[0], payloadoffsets, verbose=debug)
#print(alldata.data.shape, "x", alldata.data.dtype)
